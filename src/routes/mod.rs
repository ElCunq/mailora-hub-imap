...
use crate::persist; // correct import from crate root
...
#[derive(Deserialize)]
struct LoginReq { email: String, password: String, host: Option<String>, port: Option<u16> }

async fn login(Json(payload): Json<LoginReq>) -> impl IntoResponse {
    if payload.email.is_empty() || payload.password.is_empty() {
        return StatusCode::BAD_REQUEST.into_response();
    }
    let host = payload.host.unwrap_or_else(|| std::env::var("IMAP_HOST").unwrap_or_else(|_| "imap.example.com".into()));
    let port: u16 = payload.port.or_else(|| std::env::var("IMAP_PORT").ok().and_then(|v| v.parse().ok())).unwrap_or(993);

    let res = crate::imap::sync::initial_snapshot(&host, port, &payload.email, &payload.password).await;
    match res {
        Ok(snap) => {
            ...
            let store = ACCOUNTS.clone();
            {
                ...
                w.insert("1".into(), AccountCreds { email: payload.email.clone(), password: payload.password.clone(), host: host.to_string(), port });
            }
            if let Err(e) = persist::save_state().await { tracing::warn!("persist save error: {e}"); }
        ...
        Err(e) => {
            let msg = e.to_string();
            let error_code = if msg.contains("Application-specific password required") { "APP_PASSWORD_REQUIRED" }
                else if msg.contains("Invalid credentials") || msg.contains("authentication failed") { "INVALID_CREDENTIALS" }
                else if msg.contains("Connection reset") { "CONNECTION_RESET" }
                else { "IMAP_AUTH_FAILED" };
            Json(serde_json::json!({
                "ok": false,
...
async fn root_page() -> impl IntoResponse { Html(include_str!("../../static/index.html")) }

use axum::extract::Json as AxumJson;
use serde::Serialize;

#[derive(Deserialize)]
struct SendReq { accountId: String, to: String, subject: String, body: String }
#[derive(Serialize)]
struct SendResp { ok: bool }

async fn send_action(AxumJson(req): AxumJson<SendReq>) -> impl IntoResponse {
    let creds_opt = { let store = ACCOUNTS.read().await; store.get(&req.accountId).cloned() };
    if creds_opt.is_none() { return StatusCode::NOT_FOUND.into_response(); }
    ...
    let smtp_user = std::env::var("SMTP_USERNAME").unwrap_or_else(|_| creds_opt.as_ref().unwrap().email.clone());
    let smtp_pass = std::env::var("SMTP_PASSWORD").unwrap_or_else(|_| creds_opt.as_ref().unwrap().password.clone());
    match crate::smtp::send_simple(&smtp_host, &smtp_user, &smtp_pass, &req.to, &req.subject, &req.body) {
    ...
    sqlx::SqlitePool: axum::extract::FromRef<S>,
{
    Router::new()
        ...
        .route("/messages/:account_id/:folder", get(sync::get_folder_messages))
}