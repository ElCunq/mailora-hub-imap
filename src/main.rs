...
async fn main() -> Result<()> {
    ...
    if let Err(e) = persist::load_state().await { tracing::warn!("persist load error: {e}"); }

    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("info,mailora_hub_imap=debug")))
        .init();

    // Build a correct sqlite URL (sqlx expects sqlite://path or sqlite::memory:)
    let raw_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite://mailora_imap.db".into());
    ...
    if std::path::Path::new("migrations").exists() {
        ...
        if let Err(e) = db::run_migrations(&pool).await { tracing::warn!("migration error: {e}"); }
        if let Err(e) = db::seed_account(&pool).await { tracing::warn!("seed error: {e}"); }
        ...
        let idle_routes = Router::new()
            .route("/idle/start/:account_id", axum::routing::post(routes::idle::start_idle_watcher))
            .route("/idle/stop/:account_id", axum::routing::post(routes::idle::stop_idle_watcher))
...
fn normalize_sqlite_url(input: &str) -> String {
    ...
    if input.starts_with("sqlite://") || input.starts_with("sqlite::memory:") { return input.to_string(); }
    if input.starts_with("sqlite:") { // single colon like sqlite:foo.db -> make it sqlite://foo.db
        let rest = input.trim_start_matches("sqlite:");
        return format!("sqlite://{}", rest.trim_start_matches('/'));
    }
    if input.starts_with("file:") { return format!("sqlite://{}", input.trim_start_matches("file:")); }
    // bare path
    format!("sqlite://{}", input)
}
