...
pub async fn fetch_message_body(
    ...
    let mut messages = session.uid_fetch(&uid_str, "(UID ENVELOPE BODY.PEEK[] FLAGS)")
        .await
    ...
    let from = envelope.from
        ...
        .and_then(|addr| {
            let name = addr.name.as_ref().and_then(|n| std::str::from_utf8(n.as_ref()).ok());
            let mailbox = addr.mailbox.as_ref().and_then(|m| std::str::from_utf8(m.as_ref()).ok());
            let host = addr.host.as_ref().and_then(|h| std::str::from_utf8(h.as_ref()).ok());
