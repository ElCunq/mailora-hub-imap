...
pub async fn sync_folder_messages(
    ...
    info!("Starting sync for account {} folder {}", account.email, folder);
    ...
    let server_uids: HashSet<u32> = messages_vec
        .iter()
        .filter_map(|m| m.uid)
        .collect();
    ...
    let deleted_count = if !deleted_uids.is_empty() {
        let placeholders = deleted_uids.iter().map(|_| "?").collect::<Vec<_>>().join(",");
        let query = format!(
    ...
    if !new_uids.is_empty() {
        ...
        for chunk in new_uids.chunks(50) {
            ...
            let messages = session
                .uid_fetch(&uid_set, "(UID FLAGS ENVELOPE BODY.PEEK[HEADER] INTERNALDATE RFC822.SIZE)")
                .await
            ...
            while let Some(fetch_result) = stream.next().await {
                match fetch_result {
                    Ok(fetch) => {
                        match save_message_to_db(pool, account, folder, &fetch).await {
                            Ok(true) => new_count += 1,
                            Ok(false) => updated_count += 1,
                            Err(e) => warn!("Failed to save message UID {}: {}", fetch.uid.unwrap_or(0), e),
                        }
                    }
...
pub async fn sync_account_messages(
    ...
    info!("Syncing {} folders for {}", folder_names.len(), account.email);
