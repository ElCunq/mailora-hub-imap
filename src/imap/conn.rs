...
pub struct ImapCapabilities { pub condstore: bool, pub qresync: bool }

pub struct ImapSession { pub session: Session<tokio_util::compat::Compat<tokio_native_tls::TlsStream<TcpStream>>>, pub caps: ImapCapabilities }
...
pub async fn connect(host: &str, port: u16, user: &str, pass: &str) -> Result<ImapSession> {
    ...
    let caps = ImapCapabilities { condstore: false, qresync: false };
